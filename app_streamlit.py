...
def save_report_internally(patient_data, generated_report):
    """
    Salva os dados da triagem e o relatório gerado em um arquivo de texto.
    """
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S") # Formato: AAAA MM DD_HH MM SS
    
    # Tenta pegar o nome do paciente da primeira pergunta
    patient_name_for_file = "PacienteAnonimo"
    q1_data = patient_data.get("Pergunta 1: Qual seu nome, idade, whatsapp e cidade?", "")
    if q1_data:
        # Extrai o nome assumindo que é o primeiro item antes da primeira vírgula
        name_parts = q1_data.split(',')[0].strip()
        if name_parts:
            # Garante que o nome do arquivo seja válido (sem caracteres especiais)
            patient_name_for_file = "".join(c for c in name_parts if c.isalnum() or c == ' ').strip().replace(" ", "_").replace("__", "_")
            if not patient_name_for_file: # Caso o nome extraído seja vazio após limpeza
                patient_name_for_file = "PacienteAnonimo"

    filename_full = f"relatorio_{patient_name_for_file}_{timestamp}.txt"
    
    # Criar a pasta 'relatorios_triagem' se não existir
    ...
    full_report_content = ""
    full_report_content += f"--- RELATÓRIO DE TRIAGEM REDE ELLe - {timestamp} ---\n\n"
    full_report_content += "## Dados Coletados na Triagem:\n"
    
    # Iterar sobre os dados do paciente para salvar
    for question_key, response_value in patient_data.items():
        full_report_content += f"{question_key}: {response_value}\n"
        
    full_report_content += "\n---\n\n"
    full_report_content += "## EXAME PSÍQUICO com devolutiva Psicanalítica (Gerado pela IA):\n"
    if generated_report:
        full_report_content += generated_report
    else:
        full_report_content += "Relatório da IA não pôde ser gerado devido a erro.\n"
    full_report_content += "\n---\n"
    full_report_content += "Sessão finalizada com sucesso.\n"

    with open(filepath, "w", encoding="utf-8") as f:
        f.write(full_report_content)
    
    return filepath, full_report_content # Retorna o caminho do arquivo salvo e o conteúdo completo
...
def send_report_email(subject, body, filepath=None):
    ...
    if not SENDER_EMAIL or not SENDER_PASSWORD:
        st.error("Credenciais de e-mail não configuradas no arquivo .env. Envio de e-mail falhou.")
        return False
    ...
    try:
        ...
        st.error(f"Erro de autenticação SMTP: Suas credenciais de e-mail estão incorretas ou você precisa de uma senha de aplicativo. Erro: {e}")
        return False
    except Exception as e:
...
if 'current_step' not in st.session_state: # CORRIGIDO: de st.session_session_state para st.session_state
...
# --- Etapa de Geração e Salvamento do Relatório ---
    ...
    # Salvar o relatório e os dados do paciente internamente para Carla revisar
    if relatorio_gerado:
        st.session_state.report_filepath, st.session_state.report_content_for_email = save_report_internally(st.session_state.dados_paciente, relatorio_gerado)
        
        # Tentar enviar o e-mail
        email_subject = f"Relatório de Triagem REDE ELLe - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
        email_body = st.session_state.report_content_for_email # O conteúdo completo do relatório para o corpo do email
        
        if send_report_email(email_subject, email_body):
